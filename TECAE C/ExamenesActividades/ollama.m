...
% The element is subjected to a tensile stress σ = 100MPa in the x-direction. 
...
%% Datos 2
l = 40E-03;    % Initial length (m)
sigmaX = 200E06; % Stress in the x-direction (Pa)
sigma = [sigmaX; 0; 0]; % Stress tensor (Pa)
% (a) the fibers are aligned along the x-axis.
thetaA = 0; % Orientation angle θ (°)
% (b) the fibers are inclined to the x-axis with an orientation angle theta = 45◦.
thetaB = 30; % Orientation angle θ (°)
theta = [-30; 0; 30]; % Orientation angles

E1 = 155.0E09;  % Longitudinal modulus E1 (Pa)
E2 = 12.10E09;  % Transverse modulus E2 (Pa)
E3 = E2;       % Transverse modulus E3 (Pa)
nu23 = 0.458;  % Poisson's ratio nu23
nu12 = 0.248;  % Poisson's ratio nu12
nu13 = nu12;   % Poisson's ratio nu13
...
for i = 1:length(m)
    ...
    Tinv = inv(T); % Inverse of the transformation matrix
    S = Tinv \ (S * T); % Transformed compliance matrix (1/Pa)
    epsilon = S * sigma; % Strain tensor (Pa)

    % Calculate the changes in the 50-mm dimensions of the element
    delta_x(i) = epsilon(1) * l; % Change in the 50-mm dimension (m)
    delta_y(i) = epsilon(2) * l; % Change in the 50-mm dimension (m)
    gamma_xy(i) = epsilon(3); % Shear strain (Pa)
end

% Final lengths after deformation
final_lengths = [l + delta_x; l + delta_y]; % Final lengths (m)

% Display results
disp('AF5 - Problem 4')
disp(['Final length of the x-direction: ' num2str(final_lengths(1, :) * 1000) ' mm'])
disp(['Final length of the y-direction: ' num2str(final_lengths(2, :) * 1000) ' mm'])
fprintf('Shear strain (gamma_xy): %.2e radians\n', gamma_xy)

% Plotting the deformed element
% figure;
% hold on;
% rectangle('Position', [0, 0, l, l], 'EdgeColor', 'r'); % Original square
% for i = 1:numel(m)
%     rectangle('Position', [(m(i)-l/2)+delta_x(i), (n(i)-l/2)+delta_y(i), l, l], ...
%         'EdgeColor', 'b'); % Deformed square
%         axis equal;
%     axis([-1 1 -1 1]); % Set the axis limits for better visualization
%     xlabel('X (m)');
%     ylabel('Y (m)');
%     title('Deformed Element');
%     legend('Original', 'Deformed');
%     pause(0.00001); % Pause to visualize each step
%     hold off;
% end
